...
period_snapshot <- function(values, period, names) {
  stats <- lapply(names, function(name) round_value(values[[name]][period]))
  names(stats) <- names
  stats
}

scenario_snapshot <- function(env, scenario, period, scalar_names, vector_names = character()) {
  stats <- period_snapshot(
    setNames(lapply(scalar_names, function(name) env[[name]][scenario, ]), scalar_names),
    period,
    scalar_names
  )
...
model_path <- "references/keynote_speech_Florence/1_ECO-3IO-PC-Model.R"
model_env <- new.env()
    ...
    "baseline-run" = list(
      periods = list(
        "5" = scenario_snapshot(
          ...
          1,
          5,
          c("y", "yd", "cons", "v", "b_h", "h_h", "p_c", "temp", "emis", "co2_cum"),
          c("x", "p")
        ),
...
fixture_path <- file.path(output_dir, "eco-3io-pc.json")
...
write_fixture("eco-3io-pc", fixture)
cat(sprintf("Wrote %s\n", fixture_path))
